...
def run_multiple_linear_regression(df_data: pd.DataFrame,
    ...
    if N <= k + 1:
        raise ValueError(f"Sample size N={N} must be greater than number of predictors k={k} + 1.")
    ...
    for i in range(len(param_names)):
        coeff_table.append({
            ...
            "CI_lower": float(model.conf_int(alpha)[i, 0]),
            "CI_upper": float(model.conf_int(alpha)[i, 1])
        })
