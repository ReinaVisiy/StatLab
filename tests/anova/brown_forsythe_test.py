...
Exports: run_brown_forsythe_test
...
def run_brown_forsythe_test(groups: list, alpha: float = 0.05, lang: str = "en") -> dict:
    validate_range(alpha, 0.001, 0.5, "alpha (α)", lang=lang)
    if len(groups) < 2:
        raise ValueError("Brown-Forsythe's test requires at least 2 groups.")
    ...
    # Brown-Forsythe is Levene's test centered on the group median.
    res = levene(*parsed_groups, center="median")
    ...
    steps = [
        ...
        f"2. {tt('center_method_label', lang)}: median (Brown-Forsythe)",
        f"3. Brown-Forsythe W = {stat:.4f} (df₁ = {df1}, df₂ = {df2})",
    ...
    return {
        ...
        "assumptions": {
            "robust_to_non_normality": {
                "en": "The Brown-Forsythe test (median-centered) is robust against non-normality.",
                "fr": "Le test de Brown-Forsythe (centré sur la médiane) est robuste à la non-normalité.",
            }[lang]
        },
