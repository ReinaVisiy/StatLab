...
def run_gof_cauchy(data_input, class_edges, alpha: float = 0.05, lang: str = "en") -> dict:
    """
    Tests whether continuous data follow a Cauchy(x0, gamma) distribution.
    The Cauchy distribution has no defined mean or variance, so ordinary
    method-of-moments estimation is impossible; robust estimators are used
    instead: x0 (location) is estimated by the sample median, and gamma
    (scale) is estimated as half the interquartile range (for a Cauchy
    distribution, IQR = 2*gamma exactly).
    ...
    if gamma_hat <= 0:
        raise ValueError("Estimated scale (half the IQR) must be strictly positive.")
