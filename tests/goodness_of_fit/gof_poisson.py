...
def run_gof_poisson(data_input, alpha: float = 0.05, lang: str = "en") -> dict:
    """
    Tests whether count data follow a Poisson(lambda) distribution, with
    lambda estimated by the sample mean. Poisson support is unbounded above,
    so individual categories are used up to the largest observed value, and
    the remaining tail probability is folded into a single "X >= K" category
